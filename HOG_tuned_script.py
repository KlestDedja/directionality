...
for group in group_folders:
    ...
    for img_idx, image_file in enumerate(image_file_list):
        ...
        hog_descriptor = HOGDescriptor(orientations=45, pixels_per_cell=(64, 64), cells_per_block=(1, 1),
                                    channel_axis=-1)

        n_pixels_cell = hog_descriptor.pixels_per_cell
        n_cells_block = hog_descriptor.cells_per_block[0]*hog_descriptor.cells_per_block[1]
        ...
        if block_norm is None: # manually normalize:
            strengths = cell_signal_strengths(fd, norm_ord=1) #output shape is shape (N, M), we normalize by L1 norm.
        ...
        # threshold = np.percentile(strengths.flatten(), 55) # for now, we set this as threshold, but can improve
            ...
            fd_norm = fd
            warnings.warn(f"There is no implemented solution for background elimination"
        ...
        gradient_hist_180 = correction_of_round_angles(orientations_180_deg,
                                                       gradient_hist_180,
                                                       corr90=True, corr45=True)
...
filename = "HOG_stats_"+ str(block_norm) + "_" + str(hog_descriptor.pixels_per_cell[0]) + "pixels"
