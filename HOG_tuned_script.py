...
for group in group_folders:
    ...
    for image_file in image_file_list:
        ...
        hog_descriptor = HOGDescriptor(orientations=30, pixels_per_cell=(32, 32), cells_per_block=(1, 1),
                                    channel_axis=-1)
        fd_raw, hog_image = hog_descriptor.compute_hog(image, block_norm=None, feature_vector=False)
        ...
        strengths = cell_signal_strengths(fd) # shape (N, M)
        fd_norm = fd/strengths[:, :, np.newaxis] # normalize everything

        threshold = np.percentile(strengths.flatten(), 55) # for now, we set this as threshold, but can improve

        threshold = 1.568
        ''' assuming cell blocks of size (32, 32)
        ...
        ax4.set_title('Signal heatmap,\n masked areas in gray')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(experiments_folder, "HOG_plot_analysis", image_filename), dpi=300)
        # plt.draw()
...
df_statistics.to_csv(os.path.join(experiments_folder, 'HOG_statistics_draft.csv'))
...
'''
QUESTIONS:
- THE STAINING CAN BE LOCALLY STRONGER OR WEAKER. IS THAT IMPORTANT OR SHOULD THAT COUNT THE SAME?
        Intensity NOT important
- SIZE OF THE TISSUE ALSO MATTERS ( bigger / longer tissues -> more weight)
-
'''
