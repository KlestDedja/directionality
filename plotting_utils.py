...
def explanatory_plot_polar(image):
    ...
    ax2.set_title("Histogram of Oriented Gradients")
    ax2.bar(
        x=orientations_180_deg,
        height=list(gradient_hist.values()),
        width=180 / len(orientations_180_deg),
        edgecolor="black",
        linewidth=0.2,
    )
    ax2.set_xticks([])  # Clear existing x-ticks
    ax2.set_yticks([])  # Clear existing y-ticks
    ax2.set_xlabel("")  # Clear existing x-axis label
    ax2.set_ylabel("")  # Clear existing y-axis label
    ax2.set_xticks(
        np.arange(min(orientations_180_deg), max(orientations_180_deg) + 1, 30)
    )  # Set custom ticks every 30
